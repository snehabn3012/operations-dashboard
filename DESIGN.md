...
- The one remaining coverage gap: a drag-and-drop add/reorder flow end-to-end test (dnd-kit's pointer-based drag events are the hard part to simulate realistically in jsdom) — adversarial review flagged this specifically as implemented-but-undemonstrated. Current coverage: `config/dashboardConfig.test.ts` (role-scoped overrides are data-driven, not a role branch), `store/dashboardSlice.test.ts` (builder UX, undo/redo, history cap, in-flight-save merge logic), `lib/configValidator.test.ts` (hostile input, schema-version migration), `components/dashboard/WidgetShell.test.tsx` (loading/empty/error/success presentation), `components/dashboard/WidgetRenderer.test.tsx` (unsupported widget/data source, real loading→success and loading→error via RTK Query, sibling-widget error isolation), `components/configuration/ConfigurationPanel.test.tsx` (role switching, undo/redo buttons, save-conflict preserving the local draft, save confirmation copy, in-flight-save edit preservation), `data/mockApi.test.ts` (optimistic concurrency), `hooks/useInfiniteModules.test.tsx` (next page, end of pagination, duplicate-request prevention, failed page, retry).
